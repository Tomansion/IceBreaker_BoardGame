...
public interface IChallenger {
    // Attention:
    // Il n’y a pas forcément besoin de constructeurs
    // dans la classe MyChallenger;
    // toutefois si vous décidez d’en écrire, il faudra
    // obligatoirement qu’une version
    ...
    // @return le nom de votre équipe sous la forme "Nom1
    // − Nom2"
    String teamName();

    // L’arbitre vous signale votre rôle au début de la partie.
    // Vous pouvez préparer votre représentation interne
    // du plateau à ce moment.
    // @param role le rôle qui vous est assigné ("RED" ou "BLACK")
    void setRole(String role);

    // L’arbitre vous signale qu’il valide le dernier coup communiqué.
    // Vous pouvez jouer ce coup dans votre représentation
    // interne du plateau à ce moment.
    ...
    // Vous pouvez jouer ce coup dans votre représentation
    // interne du plateau à ce moment.
    // @param move le coup que l’adversaire vient de jouer,
    // sous la forme "D2−C2"
    void otherPlay(String move);

    // La fonction best Move n’est à implémenter que pour la partie 3 du devoir.
    ...
    // Choisissez bien.
    // @return le coup que vous comptez jouer (et que vous soumettez à la validation de l’arbitre), sous la forme"D2−C2"
    String bestMove();
    ...
    // Vous devez renvoyer l’ensemble des coups possibles pour l’un des joueurs (d’après l’état actuel du plateau dans votre représentation interne).
